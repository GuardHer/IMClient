...
public:
    ...
    string readBytes(int n) {
        ...
        auto nr = ::fread(buf.data(), 1, n, fp_);
        if (nr != n) {
    ...
    int64_t readInt64() {
        int64_t x = 0;
        auto nr = ::fread(&x, 1, sizeof(int64_t), fp_);
    ...
    int32_t readInt32() {
        int32_t x = 0;
        auto nr = ::fread(&x, 1, sizeof(int32_t), fp_);
    ...
    {
        uint8_t x = 0;
        auto nr = ::fread(&x, 1, sizeof(uint8_t), fp_);
        if (nr != sizeof(uint8_t))
            throw std::logic_error("bad uint8_t data");
        return x;
    }

    off_t skip(size_t bytes)
    {
