    ...
    , m_role(role)
{
    ...
    connect(this, &Bubble::sigChangedWidth, [=](int textWidth) {
...
//        setFixedHeight(50 + height);
        ...
        QFontMetrics fontSize(ui->textBrowser->font());
        QString myText = ui->textBrowser->toPlainText();
        int textWidth = fontSize.size(Qt::TextSingleLine, myText).width();

        emit sigChangedHeight(height + h1 + h2);  // 记得加上间隔
        emit sigChangedWidth(textWidth + 20);
    });
